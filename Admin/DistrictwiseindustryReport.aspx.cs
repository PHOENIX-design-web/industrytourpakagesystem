...
namespace industrytourpakagesystem.Admin
{
    public partial class DistrictwiseReport : System.Web.UI.Page
    {
        ...
        {
            db.fillgrid("select count(*) as count, d.districtname FROM tblindustry i INNER JOIN tbldistrict d ON i.districtid = d.districtid GROUP BY d.districtname", GridView1);

            DataTable objdatatable = db.exedatatable("select count(*) as count, d.districtname FROM tblindustry i INNER JOIN tbldistrict d ON i.districtid = d.districtid GROUP BY d.districtname");
